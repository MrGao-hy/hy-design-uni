<template>
    <view class="hy-swiper-indicator">
        ...
        >
            <view class="hy-swiper-indicator__wrapper--line__bar" :style="[lineStyle]"></view>
        </view>
        <view class="hy-swiper-indicator__wrapper" v-if="indicatorMode === 'dot'">
            <view
                ...
                :key="index"
                :class="[index === current && 'hy-swiper-indicator__wrapper--dot__active']"
...
const lineStyle = computed<CSSProperties>(() => {
    ...
    style.transform = `translateX(${addUnit(Number(current.value) * lineWidth.value)})`
...
const dotStyle = computed(() => {
    return (index: number): CSSProperties => {
        ...
        style.backgroundColor =
            index === current.value ? indicatorActiveColor.value : indicatorInactiveColor.value
        return style
    }
})
</script>
