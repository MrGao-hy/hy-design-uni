...
const REGEX = {
    ...
    EVENT_TUPLE: /\/\*\*[\s\S]*?\*\/\s*['"]?([^\s'"]+)['"]?\s*:\s*\[([^\]]*)\]/g,
    EVENT_TUPLE_NO_COMMENT: /['"]?([^\s'"]+)['"]?\s*:\s*\[([^\]]*)\]/g,
    EMIT_CALL: /emit\(['"]([\w-]+)['"]/g,
...
function extractEmitsFromVueFile(vueFilePath, componentName, customEmitsInterface) {
    ...
    if (CONFIG.DEBUG) {
            ...
            if (interfaceNameToUse) {
                ...
                if (interfaceStart !== -1) {
                    ...
                    while ((eventMatch = REGEX.EVENT_TUPLE.exec(fullInterfaceContent)) !== null) {
                        ...
                        eventInfoMap[eventName] = { description, parameters }
                        if (CONFIG.DEBUG) {
                            console.log(`提取到事件 ${eventName} 的描述: ${description}, 参数: ${parameters}`)
                        }
                    }
                    ...
                    while ((eventMatch = REGEX.EVENT_TUPLE_NO_COMMENT.exec(fullInterfaceContent)) !== null) {
                        ...
                        if (!eventInfoMap[eventName]) {
                            ...
                            if (CONFIG.DEBUG) {
                                console.log(`提取到事件 ${eventName} (无注释), 参数: ${rawParams.trim() || '-'}`)
                            }
                        }
                    }
                    ...
                    while ((eventMatch = REGEX.EVENT_WITH_COMMENT.exec(fullInterfaceContent)) !== null) {
                        ...
                        if (!eventInfoMap[eventName]) {
                            ...
                            const description = descriptionMatch
                                ? descriptionMatch[1].replace(/\*\s?/g, '').trim()
                                : ''
                            eventInfoMap[eventName] = { description, parameters: 'event' }
                            if (CONFIG.DEBUG) {
                                console.log(`[旧格式] 提取到事件 ${eventName} 的描述: ${description}`)
                            }
                        }
                    }
        ...
        // 1. 先添加从接口中提取的所有事件
        for (const [eventName, info] of Object.entries(eventInfoMap)) {
        ...
        // 2. 再添加从emit调用中提取的事件，如果已经存在则保留接口中的信息
